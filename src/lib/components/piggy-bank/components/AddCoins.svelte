<script lang="ts">
	...
	const increaseDesiredHoldAmount = () => {
		const holdDepositCeil =
			$pbState?.bankRemainingBalance > $playerState?.general?.balance
		...
		if (
			...
			$pbState.desiredHoldDepositAmount === $pbState.bankRemainingBalance
		) {
	...
	const decreaseDesiredHoldAmount = () => {
		...
		if (
			...
			$pbState.desiredHoldDepositAmount =
				$pbState.desiredHoldDepositAmount < 0 ? 0 : $pbState.desiredHoldDepositAmount;
		}
	};
</script>
