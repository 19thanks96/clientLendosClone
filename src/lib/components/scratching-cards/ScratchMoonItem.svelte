<script lang='ts'>
    ...
    let idleScratch = {
        ...
        moon: {
            forward: 'https://p2w.imgix.net/resources/client/sc/gold-close.svg?auto=compress&auto=format',
            background: 'https://p2w.imgix.net/resources/client/sc/gold-open.svg?auto=compress&auto=format'
        },
        empty: {
    ...
    onMount(async () => {
        ...
        const sc = new ScratchCard('#js--sc--container', {
            ...
            htmlBackground: `
        <div style="background-image: url('${idleScratch[color].background}');background-size: cover;border-radius: 10px;background-position: center center;" class="z-[1] bg-[length:100%_100%] bg-no-repeat w-full h-full absolute left-0 top-0 flex justify-center items-center canvas-control"><div class="absolute items-center vhHeight-[24] leading-[1.2] vhFont-[34] items-center font-semibold flex value-text" style="font-size: 20px;
        font-style: normal;
        font-weight: 600;
        line-height: normal;
        letter-spacing: 0.2px;
        text-transform: capitalize;
        color: #2A2A2A; gap: 4px;">${win ? `${coin} ${text}` : text}</div></div>`,
            clearZoneRadius: 0,
            nPoints: 100,
            pointSize: 1,
            callback: function () {
    ...
    <div class="sc__wrapper w-[75%] h-[70%]  bg-contain bg-no-repeat flex items-center justify-center bg-center  relative  ">
        ...
        {#if !$scratchState.isSratchBtnClck}
            <div id="js--sc--container"
                 class="w-full h-full bg-contain bg-no-repeat flex items-center justify-center bg-center relative  canvas-before">
        ...
        <div style="{$scratchState.isSratchBtnClck ? '' : 'opacity:0'}"
            ...
            <div class="absolute z-[1] vhHeight-[24] leading-[1.2] vhFont-[34] self-center flex items-center font-semibold value-text" style="font-size: 20px;
        font-style: normal;
        font-weight: 600;
        line-height: normal;
        letter-spacing: 0.2px;
        text-transform: capitalize;
        color: #2A2A2A;">{text}</div>
