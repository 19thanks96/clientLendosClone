    ...
    onMount(async () => {
        ...
        let loadMissionsForTutorial = () => {
            ...
            if ($playerState.tutorial && $playerState.tutorial.step === 1) {
                ...
                $playerState.mg.missions = enrichTutorialMissions($playerState.mg.missions, $playerState.tutorial, 'mission', false, $playerState.tutorial.step);
        ...
        AdapterCommunicationService.subscribeOnInitUpdates((data: PlayerDataType) => {
                if (data) {
            ...
            (missionProgressResponse) => {
                wrapAdapterResponse(missionProgressResponse?.result, () => {
                    $playerState.mg.missions.find(x => x.id === missionProgressResponse.missionId).progress = missionProgressResponse.progress;
                    $playerState.mg.missions.find(x => x.id === missionProgressResponse.missionId).status = missionProgressResponse.status;
            ...
            (missionClaimResponse) => {
                wrapAdapterResponse(missionClaimResponse?.result,
                    () => {
                        ...
                        $playerState.general.dmRed = $playerState.mg.missions.some((m) => m.status === 'completed');
            ...
            (offerClaimResponse: StoreOfferClaimResponse) => {
                wrapAdapterResponse(offerClaimResponse.result,
                    () => {
                        if (!$playerState.store.offers.find(x => x.id === offerClaimResponse.store.offerId)?.reusable) {
                            $playerState.store.offers = $playerState.store.offers.filter(x => x.id !== offerClaimResponse.store.offerId);
                        }
        ...
        AdapterCommunicationService.sendMessage({type: 'appLoaded',message:{fromWidget:isFormWidget}});
    });

    const wrapAdapterResponse = (result: {
        success: boolean,
        message: string
    }, onSuccess: () => void, onFailure: () => void = null) => {
        if (!result.success) {
